/*
Function: havoc_fnc_ConfigZeus

Description:
    Used to configure a Zeus Module. Ensures that any AI placed down by that Zeus have Dynamic Sim
    enabled and their difficulty settings configured to the Havoc Default.
...
Examples:
    (begin example)
        [this] call havoc_fnc_ConfigZeus;
    (end)
    ...
    "CuratorObjectPlaced", {
        ...
        _result  = call {
            if (_class isKindOf "Man") exitWith {
                ...
                [_Group, "Havoc Default"] remoteExec ["havoc_fnc_SetUnitSkill",2];
                [_Group, true] remoteExec ["enableDynamicSimulation",2];
            };

            if (_class isKindOf "AllVehicles") exitWith {
                {
                    [_x, "Havoc Default"] remoteExec ["havoc_fnc_SetUnitSkill",2];
