...
switch (_patrolMethod) do {
    case "ROAD": {
        _fnc_patrol_EI = compile "[(_this select 0), (_this select 3)] call havoc_fnc_RoadPatrol;";
        _fnc_patrol_EI_spec = compile "[(_this select 0), (_this select 3)] call havoc_fnc_RoadPatrol;";
        _fnc_patrol_veh = compile "[(_this select 0), (_this select 3)] call havoc_fnc_RoadPatrol;";
    ...
    case "RANDOM": {
        ...
        _fnc_pos_EI = compile "[[[(_this select 0), (_this select 1)],[]],[""water""]] call havoc_fnc_SafePos;";
        _fnc_pos_veh = compile "[[[(_this select 0), (_this select 1)], []], [""water""], { !(_this isFlatEmpty [2,-1,0.5,1,0,false,objNull] isEqualTo []) }] call havoc_fnc_SafePos;";
    };
    default {
        _fnc_patrol_EI = compile "systemChat ""Error: Unknown patrol method supplied to havoc_fnc_SpawnAI!"";";
        _fnc_patrol_EI_spec = compile "systemChat ""Error: Unknown patrol method supplied to havoc_fnc_SpawnAI!"";";
        _fnc_patrol_veh = compile "systemChat ""Error: Unknown patrol method supplied to havoc_fnc_SpawnAI!"";";
        _fnc_pos_EI = compile "systemChat ""Error: Unknown patrol method supplied to havoc_fnc_SpawnAI!"";";
        _fnc_pos_veh = compile "systemChat ""Error: Unknown patrol method supplied to havoc_fnc_SpawnAI!"";";
    };
};
...
_GetFactionArrays = compileFinal format ["call havoc_fnc_%1", _faction];
...
if !(_infList isEqualTo []) then {
       ...
       private _GarDif = _GarrisonedGroupsMax - _GarrisonedGroupsMin;
        ...
        private _garrisonedUnits = (
            [
                ...
                nil, 2,
                4, []
            ] call havoc_fnc_infantryGarrison
        );
    ...
    if (_AIReporting && { _GarrisonedGroupsMax >0 }) exitwith {systemchat format ["Havoc: INFO: %1 no Infantry Teams to select from. Step skipped.",_FactionName]};
};
///////////////////////////////////////////////////////////
// STANDARD INFANTRY
///////////////////////////////////////////////////////////
if !(_infList isEqualTo []) then {
   private _InfDif = _infMax - _infMin;
    ...
    for "_x" from 1 to (_infMin + floor(random _InfDif)) do {
        ...
        [_g, _infSkill] call havoc_fnc_SetUnitSkill;
        _units append (units _g);
    };
} else {
    if (_AIReporting && { _infMax >0 }) exitwith {systemchat format ["Havoc: INFO: %1 no Infantry Teams to select from. Step skipped.",_FactionName]};
    };
...
if !(_infaaList isEqualTo []) then {
   private _InfaaDif = _InfaaMax - _InfaaMin;
    ...
    for "_x" from 1 to (_InfaaMin + floor(random _InfaaDif)) do {
        ...
        [_g, _infaaSkill] call havoc_fnc_SetUnitSkill;
        _units append (units _g);
    };
} else {
    if (_AIReporting && { _InfaaMax >0 }) exitwith {systemchat format ["Havoc: INFO: %1 no AA Teams to select from. Step skipped.",_FactionName]};
    };
...
if !(_infatList isEqualTo []) then {
   private _InfatDif = _InfatMax - _InfatMin;
    ...
    for "_x" from 1 to (_InfatMin + floor(random _InfatDif)) do {
        ...
        [_g, _center, _radius, _waypoints] call _fnc_patrol_EI_spec;
        [_g, _infatSkill] call havoc_fnc_SetUnitSkill;
        _units append (units _g);
    };
} else {
    if (_AIReporting && { _InfatMax >0 }) exitwith {systemchat format ["Havoc: INFO: %1 no AT Teams to select from. Step skipped.",_FactionName]};
    };
...
if !(_sniperList isEqualTo []) then {
   private _SniperDif = _SniperMax - _SniperMin;
    ...
    for "_x" from 1 to (_SniperMin + floor(random _SniperDif)) do {
        ...
        [_g, _sniperSkill] call havoc_fnc_SetUnitSkill;
        _units append (units _g);
    };
} else {
    if (_AIReporting && { _SniperMax >0 }) exitwith {systemchat format ["Havoc: INFO: %1 no Sniper Teams to select from. Step skipped.",_FactionName]};
    };
   ...
   private _VehAADif = _VehAAMax - _VehAAMin;
    ...
    for "_x" from 1 to (_VehAAMin + floor(random _VehAADif)) do {
        ...
        if !(_rpos isEqualTo [0,0]) then {
            ...
            [_g, _vehaaSkill] call havoc_fnc_SetUnitSkill;
    ...
    if (_AIReporting && { _VehAAMax >0 }) exitwith {systemchat format ["Havoc: INFO: %1 no AA Vehicles to select from. Step skipped.",_FactionName]};
    };
   ...
   private _VehMRAPDif = _VehMRAPMax - _VehMRAPMin;
    ...
    for "_x" from 1 to (_VehMRAPMin + floor(random _VehMRAPDif)) do {
        ...
        if !(_rpos isEqualTo [0,0]) then {
            ...
            [_v, _g] call BIS_fnc_spawnCrew;
            [_g, _center, _radius, _waypoints] call _fnc_patrol_veh;
            [_g, _vehmrapSkill] call havoc_fnc_SetUnitSkill;
            if (random 1 >= 0.5) then { _v allowCrewInImmobile true; };
    ...
    if (_AIReporting && { _VehMRAPMax >0 }) exitwith {systemchat format ["Havoc: INFO: %1 no MRAPs to select from. Step skipped.",_FactionName]};
    };
   ...
   private _VehLightDif = _VehLightMax - _VehLightMin;
    ...
    for "_x" from 1 to (_VehLightMin + floor(random _VehLightDif)) do {
        ...
        if !(_rpos isEqualTo [0,0]) then {
            ...
            [_v, _g] call BIS_fnc_spawnCrew;
            [_g, _center, _radius, _waypoints] call _fnc_patrol_veh;
            [_g, _vehLightSkill] call havoc_fnc_SetUnitSkill;
            if (random 1 >= 0.5) then { _v allowCrewInImmobile true; };
    ...
    if (_AIReporting && { _VehLightMax >0 }) exitwith {systemchat format ["Havoc: INFO: %1 no Light Vehicles to select from. Step skipped.",_FactionName]};
    };
   ...
   private _VehHeavyDif = _VehHeavyMax - _VehHeavyMin;
    ...
    for "_x" from 1 to (_VehHeavyMin + floor(random _VehHeavyDif)) do {
        ...
        if !(_rpos isEqualTo [0,0]) then {
            ...
            [_v, _g] call BIS_fnc_spawnCrew;
            [_g, _center, _radius, _waypoints] call _fnc_patrol_veh;
            [_g, _vehHeavySkill] call havoc_fnc_SetUnitSkill;
            if (random 1 >= 0.5) then { _v allowCrewInImmobile true; };
    ...
    if (_AIReporting && { _VehHeavyMax >0 }) exitwith {systemchat format ["Havoc: INFO: %1 no Heavy Vehicles to select from. Step skipped.",_FactionName]};
    };
   ...
   private _VehRandDif = _VehRandMax - _VehRandMin;
    ...
    for "_x" from 1 to (_VehRandMin + floor(random _VehRandDif)) do {
        ...
        if !(_rpos isEqualTo [0,0]) then {
            ...
            [_v, _g] call BIS_fnc_spawnCrew;
            [_g, _center, _radius, _waypoints] call _fnc_patrol_veh;
            [_g, _vehrandSkill] call havoc_fnc_SetUnitSkill;
            if (random 1 >= 0.5) then { _v allowCrewInImmobile true; };
