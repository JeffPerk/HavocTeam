/*
Function: havoc_fnc_SpawnCivilians
...
if !(_pedPool isEqualTo []) then {
    ...
    if (_AIReporting && { _garrisonsMax > 0 }) exitwith {systemchat format ["Havoc: INFO: %1 no Civilian peds to select from. Step skipped.",_faction]};
};
...
if !(_pedPool isEqualTo []) then {
   private _pedDif = _pedMax - _pedMin;
    ...
    if (_AIReporting && { _pedMax > 0 }) exitwith {systemchat format ["Havoc: INFO: %1 no Civilian peds to select from. Step skipped.",_faction]};
};
...
if !(_motPool isEqualTo []) then {
   private _vehparkedDif = _vehparkedMax - _vehparkedMin;
    ...
    if (_AIReporting && { _vehparkedMax > 0 }) exitwith {systemchat format ["Havoc: INFO: %1 no Civilian vehicles to select from. Step skipped.",_faction]};
};
