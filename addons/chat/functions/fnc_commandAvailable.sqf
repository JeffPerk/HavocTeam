...
/*
 * Name = HAVOC_chat_fnc_commandAvailable
 * Author = Freddo
 *
 * Parameters:
...
switch (_var) do {
    case 0: { // Never available
        if true exitWith {
            systemChat FORMAT_1("HAVOC: %1 is disabled.", _command);
            _enabled = false;
        };
    };
    case 1: { // Available during safestart
        if !(call EFUNC(safestart,isActive)) exitWith {
            systemChat FORMAT_1("HAVOC: %1 is only available during Safe Start.", _command);
            _enabled = false;
        };
    };
    case 2: { // Available during safestart and after respawning
        if (
            ...
            systemChat FORMAT_1("HAVOC: %1 is only available during Safe Start and within 5 minutes of respawn.", _command);
            _enabled = false;
        };
    };
};
