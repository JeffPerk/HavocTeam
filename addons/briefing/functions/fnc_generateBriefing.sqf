...
private _briefingArray = getMissionConfigValue ["havoc_Briefing","[]"];
...
private _groupCond = _unitGroup getVariable ["havoc_Briefinglist", []];
if (_groupCond isEqualType "") then { _groupCond = call compile _groupCond; };
private _unitCond = _unit getVariable ["havoc_Briefinglist", []];
...
//Process scripts.
{
    ...
    if FILE_EXISTS(_scriptName) then {
        ...
        [_scriptName] spawn {
            params ["_scriptName"];
            uiSleep 5;
            systemChat format["[HAVOC-Briefing] Missing file: %1 ", _scriptName];
        };
    };
} forEach _indexesToTrigger;

//Do Admin briefing.
if ([] call havoc_common_fnc_isAdmin) then {
    if FILE_EXISTS("briefing\admin.sqf") then {
        call compile preprocessfilelinenumbers "briefing\admin.sqf";
    } else {
        [] spawn {
            uiSleep 5;
            systemChat "[HAVOC-Briefing] Warning admin briefing not found. Expected: MISSION_ROOT\briefing\admin.sqf";
        };
    };
};
