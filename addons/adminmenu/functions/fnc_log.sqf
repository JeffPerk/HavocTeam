...
/* ----------------------------------------------------------------------------
Function: HAVOC_adminmenu_fnc_log
...
Parameters:
    ...
    _tag - Tag that message will be prefixed with [String, defaults to "[HAVOC Log]"]
...
Examples:
    (begin example)
        ["Everything is on fire!",true,"[HAVOC Firealarm]"] call HAVOC_adminmenu_fnc_log;
    (end)
    (begin example)
        [format ["%1 did something",profileName]] call HAVOC_adminmenu_fnc_log;
    (end)
