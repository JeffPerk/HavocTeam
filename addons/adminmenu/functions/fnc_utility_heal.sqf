...
if (!isNil "ace_medical_treatment_fnc_fullHeal") then {
    {
        ...
        (format ["[HAVOC Admin Menu] Your health was restored by '%1'", profileName]) remoteExec ["systemChat", _x];
...
systemChat format ["[HAVOC Admin Menu] %1 players had their health restored", count GVAR(utilityData)];
