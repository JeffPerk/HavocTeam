 ...
 * 
 * Example:
 * ["CH View Distance", "open_settings"] call Havoc_fnc_getKeybindString;
 * 
 */
...
if (!isNil "_entry") then {
    ...
    private _modifier = 
        (if (_useShift) then {localize "STR_Havoc_KeyBinds_Shift_Modifier"} else {""}) +
        (if (_useCtrl) then {localize "STR_Havoc_KeyBinds_Ctrl_Modifier"} else {""}) +
        (if (_useAlt) then {localize "STR_Havoc_KeyBinds_Alt_Modifier"} else {""});
