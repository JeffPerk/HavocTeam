// Naming pattern: TypeSleevesOrPants_ColorShirt_ColorPants_ColorBoots
// Garment - Long Sleeves
class GVAR(Uniform_Garment_LS_BS_BP_BB): GVAR(Uniform_Base) {
    scope = 2;
    author = "Pomigit, Jonpas";
    displayName = CSTRING(Uniform_Garment_LS_BS_BP_BB);
    picture = QPATHTOF(UI\uniform_garment_ls_bs_bp_ca.paa);

    class ItemInfo: ItemInfo {
        uniformClass = QGVAR(Unit_Garment_LS_BS_BP_BB);
    };
};
class GVAR(Uniform_Garment_LS_BS_GP_BB): GVAR(Uniform_Base) {
    scope = 2;
    author = "Pomigit, Jonpas";
    displayName = CSTRING(Uniform_Garment_LS_BS_GP_BB);
    picture = QPATHTOF(UI\uniform_garment_ls_bs_gp_ca.paa);

    class ItemInfo: ItemInfo {
        uniformClass = QGVAR(Unit_Garment_LS_BS_GP_BB);
    };
};
class GVAR(Uniform_Garment_LS_OS_EP_TB): GVAR(Uniform_Base) { // O = Brown, E = Grey
    scope = 2;
    author = "Pomigit, Jonpas";
    displayName = CSTRING(Uniform_Garment_LS_OS_EP_TB);
    picture = QPATHTOF(UI\uniform_garment_ls_os_ep_ca.paa);

    class ItemInfo: ItemInfo {
        uniformClass = QGVAR(Unit_Garment_LS_OS_EP_TB);
    };
};
class GVAR(Uniform_Garment_LS_GS_GP_BB): GVAR(Uniform_Base) {
    scope = 2;
    author = "Pomigit, Jonpas";
    displayName = CSTRING(Uniform_Garment_LS_GS_GP_BB);
    picture = QPATHTOF(UI\uniform_garment_ls_gs_gp_ca.paa);

    class ItemInfo: ItemInfo {
        uniformClass = QGVAR(Unit_Garment_LS_GS_GP_BB);
    };
};
class GVAR(Uniform_Garment_LS_GS_BP_BB): GVAR(Uniform_Base) {
    scope = 2;
    author = "Pomigit, Jonpas";
    displayName = CSTRING(Uniform_Garment_LS_GS_BP_BB);
    picture = QPATHTOF(UI\uniform_garment_ls_gs_bp_ca.paa);

    class ItemInfo: ItemInfo {
        uniformClass = QGVAR(Unit_Garment_LS_GS_BP_BB);
    };
};
class GVAR(Uniform_Garment_LS_GS_EP_TB): GVAR(Uniform_Base) { // E = Grey
    scope = 2;
    author = "Pomigit, Jonpas";
    displayName = CSTRING(Uniform_Garment_LS_GS_EP_TB);
    picture = QPATHTOF(UI\uniform_garment_ls_gs_ep_ca.paa);

    class ItemInfo: ItemInfo {
        uniformClass = QGVAR(Unit_Garment_LS_GS_EP_TB);
    };
};
class GVAR(Uniform_Garment_LS_ES_EP_TB): GVAR(Uniform_Base) { // E = Grey
    scope = 2;
    author = "Pomigit, Jonpas";
    displayName = CSTRING(Uniform_Garment_LS_ES_EP_TB);
    picture = QPATHTOF(UI\uniform_garment_ls_es_ep_ca.paa);

    class ItemInfo: ItemInfo {
        uniformClass = QGVAR(Unit_Garment_LS_ES_EP_TB);
    };
};
class GVAR(Uniform_Garment_LS_ES_BP_BB): GVAR(Uniform_Base) { // E = Grey
    scope = 2;
    author = "Pomigit, Jonpas";
    displayName = CSTRING(Uniform_Garment_LS_ES_BP_BB);
    picture = QPATHTOF(UI\uniform_garment_ls_es_bp_ca.paa);

    class ItemInfo: ItemInfo {
        uniformClass = QGVAR(Unit_Garment_LS_ES_BP_BB);
    };
};
class GVAR(Uniform_Garment_LS_ES_GP_BB): GVAR(Uniform_Base) { // E = Grey
    scope = 2;
    author = "Pomigit, Jonpas";
    displayName = CSTRING(Uniform_Garment_LS_ES_GP_BB);
    picture = QPATHTOF(UI\uniform_garment_ls_es_gp_ca.paa);

    class ItemInfo: ItemInfo {
        uniformClass = QGVAR(Unit_Garment_LS_ES_GP_BB);
    };
};
class GVAR(Uniform_Garment_LS_TS_TP_TB): GVAR(Uniform_Base) {
    scope = 2;
    author = "Pomigit, Jonpas, Rory";
    displayName = CSTRING(Uniform_Garment_LS_TS_TP_TB);
    picture = QPATHTOF(UI\uniform_garment_ls_ts_tp_ca.paa);

    class ItemInfo: ItemInfo {
        uniformClass = QGVAR(Unit_Garment_LS_TS_TP_TB);
    };
};
class GVAR(Uniform_Garment_LS_GS_TP_TB): GVAR(Uniform_Base) {
    scope = 2;
    author = "Pomigit, Jonpas, Rory";
    displayName = CSTRING(Uniform_Garment_LS_GS_TP_TB);
    picture = QPATHTOF(UI\uniform_garment_ls_gs_tp_ca.paa);

    class ItemInfo: ItemInfo {
        uniformClass = QGVAR(Unit_Garment_LS_GS_TP_TB);
    };
};
class GVAR(Uniform_Garment_LS_TS_GP_BB): GVAR(Uniform_Base) {
    scope = 2;
    author = "Pomigit, Jonpas, Rory";
    displayName = CSTRING(Uniform_Garment_LS_TS_GP_BB);
    picture = QPATHTOF(UI\uniform_garment_ls_ts_gp_ca.paa);

    class ItemInfo: ItemInfo {
        uniformClass = QGVAR(Unit_Garment_LS_TS_GP_BB);
    };
};
class GVAR(Uniform_Garment_LS_OS_TP_TB): GVAR(Uniform_Base) { // O = Brown
    scope = 2;
    author = "Pomigit, Jonpas, Rory";
    displayName = CSTRING(Uniform_Garment_LS_OS_TP_TB);
    picture = QPATHTOF(UI\uniform_garment_ls_os_tp_ca.paa);

    class ItemInfo: ItemInfo {
        uniformClass = QGVAR(Unit_Garment_LS_OS_TP_TB);
    };
};
class GVAR(Uniform_Garment_LS_TS_BP_BB): GVAR(Uniform_Base) {
    scope = 2;
    author = "Pomigit, Jonpas, Rory";
    displayName = CSTRING(Uniform_Garment_LS_TS_BP_BB);
    picture = QPATHTOF(UI\uniform_garment_ls_ts_bp_ca.paa);

    class ItemInfo: ItemInfo {
        uniformClass = QGVAR(Unit_Garment_LS_TS_BP_BB);
    };
};
